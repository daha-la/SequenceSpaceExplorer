...
> **Implementation note (was a bug, now fixed):** `common.merge_columns` previously aborted on a reserved-column (`id`/`Sequence`/`query`) name collision **unconditionally**, before checking `force` — making the `--force` override this section requires unreachable for reserved columns specifically. This is now **fixed**: the reserved-name abort is gated on `not force`, so force-overwriting a reserved column behaves exactly like any other column (refused without `--force`, full-column replace with it). Note the asymmetry with `drop_columns`: force-*overwriting* a reserved column is allowed, but force-*dropping* one via `drop_columns` is still refused unconditionally (that path has no legitimate use). Covered by `tests/test_common.py::test_merge_reserved_column_replaced_with_force`.
...
| **Reserved-column force-overwrite bug** | FIXED (§4, §10.6) | `merge_columns` used to abort on an `id`/`Sequence`/`query` name collision before checking `force`. Now gated on `not force`, so reserved columns honor the same `--force` rule as any column. Regression-guarded by `tests/test_common.py`. |
