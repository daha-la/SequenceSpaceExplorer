...
EMBEDDING_SUFFIX = ".emb.tsv"
NORMALIZED_SUBDIR = "normalized"


def resolve_embedding_cache(entry_dir, value=None, *, prefer_normalized=True):
    """Resolve which embedding cache a downstream tool should read.

    Returns (path, is_normalized). This is the single place the pipeline decides
    its vector geometry, so distance, clustering, and any future embedding
    consumer all agree.

    Caches live in <entry_dir>/embeddings/. `value` optionally names one (a tag,
    a filename, or a direct path); when omitted, the sole raw cache in
    embeddings/ is auto-detected (aborts if zero or several exist).

    When `prefer_normalized` and an L2-normalized sibling exists at
    embeddings/normalized/<name>, that path is returned instead, so everything
    downstream acts on the same normalized vectors. If no normalized cache
    exists, the raw cache is used. Pass prefer_normalized=False to force raw.
    The normalized/ directory is kept out of the auto-detect glob on purpose, so
    it never counts as a second selectable cache.
    """
    emb_dir = Path(entry_dir) / "embeddings"
    if value:
        supplied = Path(value)
        if supplied.exists():
            raw = supplied
        else:
            name = value if value.endswith(EMBEDDING_SUFFIX) else f"{value}{EMBEDDING_SUFFIX}"
            raw = emb_dir / name
            if not raw.exists():
                abort(f"embedding cache not found: {raw}")
    else:
        candidates = sorted(emb_dir.glob(f"*{EMBEDDING_SUFFIX}"))
        if not candidates:
            abort(f"no embedding caches found in {emb_dir}")
        if len(candidates) > 1:
            names = [p.name.removesuffix(EMBEDDING_SUFFIX) for p in candidates]
            abort(f"multiple embedding caches found: {names}. "
                  "Select one with --embedding.")
        raw = candidates[0]

    # A path pointed straight at the normalized/ copy is already normalized.
    if raw.parent.name == NORMALIZED_SUBDIR:
        return raw, True
    if prefer_normalized:
        norm = raw.parent / NORMALIZED_SUBDIR / raw.name
        if norm.exists():
            return norm, True
    return raw, False
